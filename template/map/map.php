    ...
    <body style="background-repeat: repeat;background: linear-gradient( <?=$gradientColorTop?>, <?=$gradientColorBottom?> ), url('<?=$backgroundPictureFilename?>'); background-attachment: fixed;">
        ...
        <div id="mapSpace">
            ...
            <div class="mainMapWrapper">
                <? if ($usi): ?>
                <div class="col-preview colIncoming" hash="">
                    <? $this->view('partial/incomingColumn',['pics'=>$pics]); ?>                
                </div>
                <? endif; ?>
                
                <div class="col-preview colMarker" hash="" ajax-allowed-to-act="">
                <?= \Components\Storage::get('doNotShowTooltips')?'':'<a class="column-label"> Marker\'s gallery </a>';?>
                </div>
                <? $mapUrl = $this->map ? '/'.\Components\Config::get('folders')['maps'] . '/' . $this->map->map_filename : \Components\Config::get('folders')['maps'] . '/' . \Components\Config::get('no_map_filename') ?>
                <div id="map" class="<?= $this->map ? '' : 'no-map' ?>" map-url="<?=$mapUrl?>" hash=""> 
                    <span class="map-tooltip">Click on the map to place a marker</span>
                </div>
            </div>
        </div>
